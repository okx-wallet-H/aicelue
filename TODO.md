...
## 待办（风控评分系统）
- [ ] 九维度行情评分系统落地（总分100分，50分中性线，高于50偏多，低于50偏空）
- [ ] 执行规则：杠杆固定四档（20x/50x/80x/100x），根据评分区间自动选择
- [ ] 执行规则：仓位、保证金、追加保证金、止损（收益额方式）、移动止盈完整公式
- [ ] 接入智谱AI（glm-4-plus）专门做评分推理，喂行情数据输出分数
- [ ] 通义千问+智谱双模型评分对比，跑数据验证谁更准
- [ ] 数据大屏同步：评分结果、双模型对比、交易执行记录实时展示
- [ ] 积累交易数据，后续微调风控系数
