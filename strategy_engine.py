...
class StrategyEngine:
    ...
    def _strategy_scores(self, state: str, tf_indicators: dict[str, dict[str, float]]) -> dict[str, float]:
        ...
        return {
            "trend_following": 1.0 if state in {"强势上涨", "弱势上涨", "强势下跌", "弱势下跌"} else 0.0,
            "mean_reversion": 1.0 if state == "区间震荡" and 30 <= m15["rsi14"] <= 70 else 0.0,
            "breakout": 1.0 if m15["bollinger_width"] >= h1["bollinger_width"] * 0.95 else 0.0,
            "momentum_confirmation": 1.0
            if (h1["ema20"] > h1["ema60"] and m15["ema20"] > m15["ema60"])
            or (h1["ema20"] < h1["ema60"] and m15["ema20"] < m15["ema60"])
            else 0.0,
        }
    ...
    def _leverage(self, symbol: str, market_state: str) -> int:
    ...
    def _close_signal(
        ...
        if pos_side == "buy":
            ...
            if position_snapshot["upl_ratio"] >= 0.045 and m15["rsi14"] >= 74 and m15["ema20"] < m15["ema60"]:
                return "CLOSE_LONG", "多单已有浮盈且15M出现转弱，执行保护性止盈。"
        ...
        if pos_side == "sell":
            ...
            if position_snapshot["upl_ratio"] >= 0.045 and m15["rsi14"] <= 26 and m15["ema20"] > m15["ema60"]:
                return "CLOSE_SHORT", "空单已有浮盈且15M出现转强，执行保护性止盈。"
    ...
    def decide(
        ...
        leverage = self._leverage(symbol_snapshot["symbol"], state_name)
